...
module.exports = function (RED) {
  function addInstancedMesh(config) {
    ...
    if (settings.create == 'init') {
      ...
      let mesh = wom.create('instancedmesh', settings.options);
      mesh.on('created', () => {
      ...
      node.on('input', (msg) => {
        ...
        let mesh = wom.create('instancedmesh', _.merge(options, msg.payload));
        mesh.on('created', () => {
          node.send({ payload: mesh });
        });
        meshes.push(mesh);
  ...
  RED.nodes.registerType('add-instanced-mesh', addInstancedMesh);
};
