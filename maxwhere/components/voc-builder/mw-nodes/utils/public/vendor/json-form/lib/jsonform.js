...
(function (serverside, global, $, _, JSON) {
  ...
  jsonform.fieldTemplate = function (inner) {
    return (
      ...
      '<% }%>' +
      'class="form-row jsonform-error-<%= keydash %>' +
  ...
  var inputFieldTemplate = function (type) {
    return {
      template:
        ...
        '<% if (node.schemaElement.minimum !== "undefined") { %>' +
        ' min=<%= node.schemaElement.minimum %>' +
        '<% } %>' +
        '<% if (node.schemaElement.maximum !== "undefined") { %>' +
  ...
  jsonform.elementTypes = {
    ...
    fieldset: {
      template:
        '<fieldset class="form-row jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
        '<% if (id) { %> id="<%= id %>"<% } %>' +
        '>' +
        '<% if (node.title || node.legend) { %><legend><%= node.title || node.legend %></legend><% } %>' +
        '<% if (elt.expandable) { %><div class="form-row"><% } %>' +
        '<%= children %>' +
    ...
    selectfieldset: {
      template:
        ...
        '<a id="<%= node.id %>"></a><% } %>' +
        '<div class="tabbable">' +
        '<div class="form-row<%= node.formElement.hideMenu ? " hide" : "" %>">' +
      ...
      onBeforeRender: function (data, node) {
        ...
        var elt = node.formElement;
        var tabs =
          '<select class="nav"' + (node.disabled ? ' disabled' : '') + '>';
        _.each(children, function (child, idx) {
          tabs +=
            '<option data-idx="' +
            idx +
